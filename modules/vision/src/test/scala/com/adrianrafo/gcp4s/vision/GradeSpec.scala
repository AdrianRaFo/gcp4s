...
class GradeSpec extends AnyFlatSpec with Matchers {
  ...
  "VisionFace" should "return the expected emotion" in {
    val joy = VisionFace(Grade.Likely, Grade.Unlikely, Grade.Unlikely, fakePosition)
    ...
    val surprise = VisionFace(Grade.Unlikely, Grade.Likely, Grade.Unlikely, fakePosition)
    ...
    val anger = VisionFace(Grade.Unlikely, Grade.Unlikely, Grade.Likely, fakePosition)
    ...
    val joyAndSurprise = VisionFace(Grade.VeryLikely, Grade.VeryLikely, Grade.Likely, fakePosition)
    ...
    val surpriseAndAnger = VisionFace(Grade.Likely, Grade.VeryLikely, Grade.VeryLikely, fakePosition)
    ...
    VisionFace(Grade.Likely, Grade.Likely, Grade.Likely, fakePosition).isExpressionless shouldBe true
  }
